"""Steps 4-7: API key / secret / password / private-key detection.

The regex+entropy scan runs once per pipeline run (memoized in ctx.cache) and
each of the four harness steps below just filters that single scan by
category — running four separate full-file scans would be wasteful busywork
...
def combined_secret_detection(ctx: PipelineContext) -> StepResult:
    """Graph Engineering's Secret Detection Agent: one node covering all four
